...
db              = client.bonds
collection = db.matured_bond_ytm_2018

query           =  collection.find({},{"_id":0})
bonds_ytm= pd.DataFrame(list(query))

default_collection = db.default_ytm
query = default_collection.find({},{'_id':0,'issuer':1})
default_issuer = pd.DataFrame(list(query))
default_issuer.drop_duplicates(inplace=True)
...
#bonds_ytm.merge(default_issuer,on='issuer',how='inner')
#删除已到期但是发生违约的债券发行主体
bonds_ytm =  bonds_ytm[~bonds_ytm['issuer'].isin(default_issuer['issuer'])]
#bonds_ytm.reset_index(inplace=True)
...
def export():
    ...
    data = data[[ 'name','defendant', 'shixin', 'zhixing','std','df']]
    data.to_excel("ytm_std_samples.xlsx")
