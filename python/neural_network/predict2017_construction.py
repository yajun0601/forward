...
def default_sample():
    code = db.default_ratios.find({'$and' :[{'INDUSTRY_CSRC12':'建筑业'},{"rptDate" : "20151231"}]},{'_id':0,'code':1,'COMP_NAME':1}) #,'COMP_NAME':1
...
#shape(report2015)
#shape(report2016)
query = db.default_ratios.find({'$and' :[{'INDUSTRY_CSRC12':'建筑业'},{"rptDate" : "20141231"}]},{'_id':0,'code':1})
...
net.add(Dense(input_dim = 64, output_dim = 32)) #添加输入层（3节点）到隐藏层（10节点）的连接
net.add(Activation('relu')) #隐藏层使用relu激活函数
#net.add(Dropout(0.5)) # over fitting
#net.add(Dense(input_dim = 50, output_dim = 20)) #添加输入层（3节点）到隐藏层（10节点）的连接
#net.add(Activation('relu')) #隐藏层使用relu激活函数
#net.add(Dropout(0.5)) # over fitting
net.add(Dense(input_dim = 32, output_dim = 1)) #添加隐藏层（10节点）到输出层（1节点）的连接
...
query = db.default_ratios.find({'$and' :[{'INDUSTRY_CSRC12':'建筑业'},{"rptDate" : "20151231"}]},{'_id':0,'code':1,'COMP_NAME':1}) #,'COMP_NAME':1
