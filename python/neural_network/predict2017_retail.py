...
report = financial_report.dropna(axis = 1, how='any', thresh=int(len(financial_report)/3))                   
...
#net.compile(loss = 'binary_crossentropy', optimizer = 'rmsprop', class_mode = "binary",metrics=['accuracy']) #编译模型，使用adam方法求解
m,n = np.shape(train)
net.fit(train[:,:n-1], train[:,-1], nb_epoch=200, batch_size=1) #训练模型，循环1000次
