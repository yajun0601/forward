...
net.add(Activation('tanh')) #隐藏层使用relu激活函数
net.add(Dropout(0.5)) # over fitting
net.add(Dense(input_dim = 32, output_dim = 32)) #添加隐藏层1
net.add(Activation('tanh')) #隐藏层使用relu激活函数
net.add(Dropout(0.5)) # over fitting
net.add(Dense(input_dim = 32, output_dim = 16)) #添加隐藏层2
net.add(Activation('tanh')) #隐藏层使用relu激活函数
net.add(Dropout(0.5)) # over fitting
...
net.compile(loss = 'mean_squared_error', optimizer = sgd) #编译模型，使用adam方法求解
m,n = np.shape(train)
net.fit(train[:,:n-1], train[:,n-1], nb_epoch=150, batch_size=1) #训练模型，循环1000次
score = net.evaluate(test[:,:n-1], test[:,n-1],batch_size=16)
print(score)
