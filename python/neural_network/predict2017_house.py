...
def default_sample():
    code = db.default_ratios.find({'$and' :[{'INDUSTRY_CSRC12':industr},{"rptDate" : "20151231"}]},{'_id':0,'code':1,'COMP_NAME':1}) #,'COMP_NAME':1
    industry = pd.DataFrame(list(code)) # 获取制造业的代码 
...
#    make traing set from 2016
    ...
    print('违约的%s：'%(industr))
    print(industry.merge(real_df,on='code'))
    
    real_df_ratio = real_df.merge(ratios, on='code')
    real_df_ratio = real_df_ratio.dropna(axis=0,how='any',thresh=2)
    real_df_manufacture = real_df_ratio.merge(industry,on='code')
    real_df_manufacture['df'] = 1.0
    
    result = industry.merge(ratios,on='code')
...
query = db.default_ratios.find({'$and' :[{'INDUSTRY_CSRC12':industr},{"rptDate" : "20141231"}]},{'_id':0,'code':1})
industry = pd.DataFrame(list(query)) #Convert the input to an array.
def get_report():
    ...
    #先求出的集合
    financial_report = financial_report.merge(industry, on='code')    
    #对集合中数据缺失超过1/3的列 删除
    report = financial_report.dropna(axis = 1, how='any', thresh=int(len(financial_report)/4))     
    #剩下的列中的空值用这一列平均值进行填充              
    report = report.fillna(report.mean())
    return report[report['rptDate'] == '20151231'],report[report['rptDate'] == '20161231']

sample,real_df_manu = default_sample()
...
m,n = np.shape(train)
net.fit(train[:,:n-1], train[:,n-1], nb_epoch=5000, batch_size=1) #训练模型，循环1000次
...
query = db.default_ratios.find({'$and' :[{'INDUSTRY_CSRC12':industr},{"rptDate" : "20151231"}]},{'_id':0,'code':1,'COMP_NAME':1}) #,'COMP_NAME':1
industry = pd.DataFrame(list(query))
industry = industry.merge(d2016,on='code')
print(industry[industry['df']==1]['COMP_NAME'])
industry[industry['df']==1].to_excel("finance2017.xlsx")
