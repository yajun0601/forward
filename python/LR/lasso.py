...
#    df = df.drop(zeros.index,axis=0)
    ...
    rate_type,rate_dict = transcoding(df[5]) # 利率类型
    df[6] = standard(df[6]) #每年付息次数
#    market,market_dict = transcoding(df[7]) #上市地点
    platform,platform_dict = transcoding(df[8]) #交易平台
    df[9] = df[9].apply(map_01)  # 是否含权债
    df[11] = df[11].apply(map_01) # 是否上市公司
    nature,nature_dict = transcoding(df[12]) #公司属性
...
#    feature_cols = ['债券期限', '每年付息次数','是否含权债','是否上市公司','大股东持股比例','发行时主体评级','发行时主体评级展望','存量债券余额']
...
#files = ['债券成交量3月.xlsx','债券成交量4月.xlsx','债券成交量5月.xlsx','债券成交量6月.xlsx','201703_06.xlsx']
...
print(params.T.round(4))
params.T.round(4).to_csv('params.csv')
