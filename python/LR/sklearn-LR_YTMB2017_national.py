...
def get_sample():
    ...
    query = db.ytmb_std_nantional_samples.find({"$or":[{'df':1},{'df':0}]},{'_id':0}) #,'name':1
...
def get_sample_standard():
    ...
    query = db.ytmb_std_nantional_samples.find({"$or":[{'df':1},{'df':0}]},{'_id':0})
    record = pd.DataFrame(list(query))
    client.close()
...
def predict_result(modle):
    ...
    query = db.ytmb_std_nantional_samples.find({"$nor":[{'df':1},{'df':0}]},{'_id':0})
    record = pd.DataFrame(list(query))
