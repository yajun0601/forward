...
#    errorCount = 0; numTestVec = 0.0
    ...
    default_num = len(testSet[testSet['df'] == 1])
    normal_num = len(testSet[testSet['df'] == 0])
...
if __name__ == "__main__":
    ...
    standard =  StandardScaler().fit_transform(record.values)
    record_sd = pd.DataFrame(standard)
    train_data = np.mat(pd.DataFrame(standard))
    train_target = df.values
    record_sd['df'] = df
    
    default = record_sd[record_sd['df']==1]
    normal = record_sd[record_sd['df']==0] #.sample(len(default)*2)
