...
#    rates=['AAA','AA+','AA','AA-','A+','A','A-','BBB+','BBB','BBB-','BB+','BB','BB-','B+','B','B-','CCC+','CCC','CCC-','CC+','CC','CC-','DDD']
    rates=['AAA','AA','A','BBB','BB','B','CCC','CC','C']
...
def read_data_sets(train_dir,
  ...
  qv = db.PROCESSED_SET.find({'rptDate':'20161231'},{'_id':0,'rptDate':0})#.limit(100) 
  financial_report = pd.DataFrame(list(qv)) # 获取技术性违约样本，从 2015年的财报中选择
  ratings = get_rating()
  ratios = financial_report.merge(ratings, on='code')
  rating = ratios.pop('LATESTISSURERCREDITRATING2').apply(map_rate)
  labels = dense_to_one_hot(rating,10)
  data = ratios.drop(['code','rptDate'], axis=1)
  zz = np.zeros([len(data),1])
